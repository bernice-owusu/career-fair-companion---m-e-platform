...
const STATUS_META: Record<QuestionStatus, { label: string; className: string; icon: React.ReactNode }> = {
  ...
  reviewed: { label: 'Reviewed', className: 'bg-orange/15 border border-orange/40 text-navy', icon: <Check className="w-3 h-3" /> },
  answered: { label: 'Answered', className: 'bg-teal/15 border border-teal/40 text-teal', icon: <CheckCircle2 className="w-3 h-3" /> },
  archived: { label: 'Archived', className: 'bg-slate-100 border border-slate-300 text-slate-500', icon: <XCircle className="w-3 h-3" /> },
};
...
export const QuestionsScreen: React.FC<QuestionsScreenProps> = ({ event, participant, onBack }) => {
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-6 pb-24 animate-fadeIn">
      <button onClick={onBack} className="flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-navy transition mb-4">
      ...
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-navy tracking-tight">Ask a Question</h1>
      ...
      {/* Ask form */}
      <form onSubmit={handleSubmit} className="bg-white border border-slate-100 rounded-lg p-4 shadow-card space-y-3 mb-6">
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full py-3.5 px-5 rounded-full bg-orange hover:bg-orange/90 text-white font-bold text-sm transition-colors flex items-center justify-center gap-2 disabled:opacity-60"
        >
      ...
      {questions.length === 0 ? (
        <div className="text-center py-10 bg-cream border border-slate-100 rounded-lg">
          <MessageSquare className="w-8 h-8 text-slate-300 mx-auto mb-2" />
          <p className="text-xs text-slate-500">You haven't asked any questions yet.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {questions.map(q => {
            ...
            return (
              <div key={q.id} className="bg-white border border-slate-100 rounded-lg p-4 shadow-card">
                <div className="flex items-start justify-between gap-3">
                  <p className="text-sm text-navy font-semibold leading-relaxed">{q.question}</p>
                ...
                {q.answer && (
                  <div className="mt-3 bg-teal/10 border border-teal/30 rounded-md p-3">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-teal mb-1">Response from facilitators</p>
                    <p className="text-xs text-navy leading-relaxed">{q.answer}</p>
                  </div>
                )}
              </div>
            );
