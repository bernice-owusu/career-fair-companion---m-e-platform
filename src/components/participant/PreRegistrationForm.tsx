...
import { ArrowLeft, ArrowRight, CheckCircle2, Info, ShieldCheck, Star, Upload, X, FileText } from 'lucide-react';
...
const STEP_FIELDS: string[][] = [
  ['fullName', 'yearOfCompletion', 'email', 'phone', 'psghRegistrationNumber', 'currentJobTitle'],
  ['currentAreaOfPractice', 'currentAreaOther', 'regionOfResidence'],
  ['idealCareerPath', 'idealCareerPathOther', 'careerFairExpectations', 'careerFairExpectationsOther'],
  ['careerTracks'],
  ['skillsLabResumeAssistance', 'skillsLabResumeAssistanceOther', 'cv'],
...
const selectionClass = (selected: boolean) =>
  `flex items-center gap-2.5 p-2.5 rounded-xl border cursor-pointer transition text-left ${
    selected ? 'bg-orange/15 border-orange text-white' : 'bg-navy/60 border-mist/15 text-mist/80 hover:border-mist/30'
  }`;
...
const Section: React.FC<SectionProps> = ({ num, title, subtitle, children }) => (
...
const Label: React.FC<{ req?: boolean; children: React.ReactNode }> = ({ req, children }) => (
...
export const PreRegistrationForm: React.FC<PreRegistrationFormProps> = ({ onSubmit, onCancel }) => {
  ...
  const toggleMulti = (field: 'careerFairExpectations' | 'careerTracks', value: string) => {
  ...
  const handleCvFile = (file?: File | null) => {
    ...
    if (!file) {
      update('cvUploaded', false);
      update('cvFileName', '');
      return;
    }
    if (file.type !== 'application/pdf') {
      const msg = 'Only PDF files are accepted. Please upload a PDF copy of your CV.';
      setCvError(msg);
      setErrors(prev => ({ ...prev, cv: msg }));
      return;
    }
    if (file.size > MAX_CV_SIZE_MB * 1024 * 1024) {
      const msg = `Your file is larger than ${MAX_CV_SIZE_MB} MB. Please upload a smaller PDF.`;
      setCvError(msg);
      setErrors(prev => ({ ...prev, cv: msg }));
      return;
    }
  ...
  const computeErrors = (): Record<string, string> => {
    ...
    if (!form.yearOfCompletion.trim()) nextErrors.yearOfCompletion = REQUIRED_MSG;
    if (!form.email.trim()) {
    ...
    if (!form.psghRegistrationNumber.trim()) {
      ...
      nextErrors.psghRegistrationNumber = 'Registration numbers should contain numbers only.';
    }
    if (!form.currentJobTitle.trim()) nextErrors.currentJobTitle = REQUIRED_MSG;

    if (form.currentAreaOfPractice === 'Other' && !form.currentAreaOther.trim()) {
      nextErrors.currentAreaOther = OTHER_MSG;
    }
    if (form.idealCareerPath === 'Other' && !form.idealCareerPathOther.trim()) {
      nextErrors.idealCareerPathOther = OTHER_MSG;
    }
    if ((form.careerFairExpectations || []).includes('Other') && !form.careerFairExpectationsOther.trim()) {
      nextErrors.careerFairExpectationsOther = OTHER_MSG;
    }
    if (form.skillsLabResumeAssistance === 'Other' && !form.skillsLabResumeAssistanceOther.trim()) {
      nextErrors.skillsLabResumeAssistanceOther = OTHER_MSG;
    }
    if (form.heardAboutCareerFair === 'Other' && !form.heardAboutCareerFairOther.trim()) {
  ...
  const renderFieldError = (key: string) =>
    errors[key] ? (
      <p data-field-error className="text-[11px] text-rose-400 font-semibold mt-1">{errors[key]}</p>
    ) : null;

  if (showLanding) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-10 sm:py-14 animate-fadeIn">
        <div className="text-center">
          <div className="inline-flex items-center gap-2 border border-orange/30 bg-orange/10 text-orange text-[11px] font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-6">
            ...
            Third Edition · Pharmacy Council AGM
          </div>

          <h1 className="text-3xl sm:text-5xl font-black text-white tracking-tight leading-tight">
            NEXUS CAREER FAIR
            <span className="block text-orange mt-1">REGISTRATION 2026</span>
          </h1>

          <p className="text-base sm:text-lg text-mist/90 font-semibold mt-6 max-w-xl mx-auto leading-relaxed">
            Are you a young pharmacist seeking to explore unconventional career paths while networking with
            industry professionals?
          </p>
        </div>

        <div className="mt-10 bg-navy/90 border border-mist/15 rounded-3xl p-6 sm:p-8 shadow-xl">
          <p className="text-sm sm:text-base text-mist/85 leading-relaxed">
            Join us for the third edition of the Nexus Career Fair at the AGM on{' '}
            <span className="font-bold text-white">Friday, September 25 at 1:00 PM GMT</span> on the{' '}
            <span className="font-bold text-white">University of Ghana campus in Accra</span>.
          </p>
          <p className="text-sm sm:text-base text-mist/85 leading-relaxed mt-4">
            Discover exciting opportunities, gain valuable insights, and enhance your career prospects.
            Don&apos;t miss this chance to connect with leading experts.{' '}
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-6">
            <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-mist/80 border border-mist/20 rounded-full px-3 py-1.5">
              <CheckCircle2 className="w-3.5 h-3.5 text-teal" /> Friday · Sep 25 · 2026
            </span>
            <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-mist/80 border border-mist/20 rounded-full px-3 py-1.5">
              <CheckCircle2 className="w-3.5 h-3.5 text-teal" /> 1:00 PM GMT
            </span>
            <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-mist/80 border border-mist/20 rounded-full px-3 py-1.5">
              <CheckCircle2 className="w-3.5 h-3.5 text-teal" /> University of Ghana · Accra
            </span>
          </div>
        </div>
        ...
        <div className="mt-8 space-y-3">
          ...
          <p className="text-center text-[11px] text-mist/60">
            Pre-register to receive your personal registration code — it takes about 3 minutes.
          </p>
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-6 pb-32 animate-fadeIn">
      ...
      >
        ...
        <span>{step === 0 ? 'Back to Home' : 'Back'}</span>
      </button>
      ...
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-mist/60 mt-0.5">
            Complete this form to pre-register for the Nexus Career Fair. Your answers help us prepare a more
            personalised event and inform the M&E team.
          </p>
        </div>
      </div>
      ...
      <div className="mb-5 bg-navy/90 border border-mist/15 rounded-2xl p-4 shadow-md">
        <div className="flex items-end justify-between gap-3 mb-2">
          <div>
            ...
            <span className="text-sm font-bold text-white">{STEP_TITLES[step]}</span>
          </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" key={step}>
        ...
        <Section num="1" title="Personal Information">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-mist/60 mt-1">Your registration code will be sent to this email.</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={e => update('psghRegistrationNumber', e.target.value.replace(/[^0-9]/g, ''))}
                className={`${baseInputClass} font-mono`}
              />
              <p className="text-[11px] text-mist/60 mt-1 flex items-center gap-1">
                <Info className="w-3 h-3 shrink-0" />
                You can enter 0000 if you do not have a PSGH registration number yet.
              </p>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Community Pharmacist or Student"
        ...
        <>
        {/* SECTION 2: CURRENT AREA OF PRACTICE */}
        <Section num="2" title="Current Area of Practice" subtitle="Where do you currently work or practice?">
          <div className="space-y-2">
            {AREA_OF_PRACTICE_OPTIONS.map(o => (
              ...
              >
                <span className="flex-1 text-xs sm:text-sm font-semibold">{o}</span>
                {form.currentAreaOfPractice === o && <CheckCircle2 className="w-4 h-4 text-orange" />}
              </button>
            ))}
          </div>
        ...
        <Section num="4" title="Career Goals & Interests">
          <div className="space-y-3">
            <div>
              <Label>Which area of the pharmaceutical world do you want to build your career in?</Label>
              <div className="space-y-2">
                {CAREER_PATH_OPTIONS.map(o => (
                  ...
                  >
                    <span className="flex-1 text-xs sm:text-sm font-semibold">{o}</span>
                    {form.idealCareerPath === o && <CheckCircle2 className="w-4 h-4 text-orange" />}
                  </button>
                ))}
              </div>
            </div>
            {form.idealCareerPath === 'Other' && (
              <div className="bg-navy/70 border border-mist/15 rounded-xl p-3.5 space-y-1.5 animate-fadeIn">
                <Label req>Please specify your ideal career path or industry.</Label>
                <input
                  type="text"
                  placeholder="Describe your ideal career path"
                  value={form.idealCareerPathOther}
                  onChange={e => update('idealCareerPathOther', e.target.value)}
                  className={baseInputClass}
                />
        ...
        <Section num="5" title="Career Fair Expectations" subtitle="What do you hope to gain from the career fair? (Select all that apply)">
          <div className="space-y-2">
            {CAREER_FAIR_EXPECTATIONS_OPTIONS.map(o => {
              const selected = (form.careerFairExpectations || []).includes(o);
              return (
                <button
                  type="button"
                  key={o}
                ...
                >
                  <span className="flex-1 text-xs sm:text-sm font-semibold">{o}</span>
                  {selected && <CheckCircle2 className="w-4 h-4 text-orange" />}
                </button>
              );
            })}
          </div>
          {(form.careerFairExpectations || []).includes('Other') && (
            <div className="bg-navy/70 border border-mist/15 rounded-xl p-3.5 space-y-1.5 animate-fadeIn">
              ...
              <input
                ...
                onChange={e => update('careerFairExpectationsOther', e.target.value)}
                className={baseInputClass}
              />
        ...
        {step === 3 && (
        <Section num="6" title="Career Track Interest" subtitle="Select all the career tracks you are interested in (12 available)">
            ...
            {CAREER_TRACK_OPTIONS.map(t => {
              ...
              return (
                ...
                >
                  ...
                  {selected && <CheckCircle2 className="w-4 h-4 text-orange shrink-0" />}
                </button>
              );
            })}
          </div>
        ...
        {step === 4 && (
        <Section num="7" title="Skills Lab" subtitle="The Skills Lab is a side event taking place during the career fair.">
          <div className="space-y-5">
            <div>
              <Label>Would you like assistance with your résumé/CV or cover letter at the Skills Lab?</Label>
              <div className="flex flex-wrap gap-2">
                {['Yes', 'No', 'Other'].map(o => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold border transition ${form.skillsLabResumeAssistance === o
              ...
              {form.skillsLabResumeAssistance === 'Other' && (
                <div className="mt-2.5 bg-navy/70 border border-mist/15 rounded-xl p-3.5 space-y-1.5 animate-fadeIn">
                  <Label req>Please specify.</Label>
                  <input
                    type="text"
                    placeholder="e.g. Cover letter review only"
                    value={form.skillsLabResumeAssistanceOther}
                    onChange={e => update('skillsLabResumeAssistanceOther', e.target.value)}
                    className={baseInputClass}
                  />
                  {renderFieldError('skillsLabResumeAssistanceOther')}
                </div>
              )}
            </div>

            <div>
              <Label>How would you rate the current quality of your résumé/CV?</Label>
              <div className="flex items-center justify-between gap-2">
                {([1, 2, 3, 4, 5] as const).map(n => (
                  ...
                  >
                    <Star className={`w-4 h-4 ${form.resumeQuality !== undefined && form.resumeQuality >= n ? 'fill-orange text-orange' : ''}`} />
            ...
            <div>
              <Label>Would you like to upload a copy of your CV for the facilitators? (Optional)</Label>
              <label className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border border-dashed border-mist/25 bg-navy/60 text-mist/70 text-xs font-bold cursor-pointer hover:border-orange hover:text-white transition">
                <Upload className="w-4 h-4" />
                <span>{form.cvFileName || `Choose PDF (max ${MAX_CV_SIZE_MB} MB)`}</span>
                <input
                  type="file"
              ...
              {form.cvUploaded && form.cvFileName && (
                <div className="flex items-center justify-between gap-2 mt-2 bg-teal/15 border border-teal/40 rounded-xl px-3 py-2 text-xs text-mist">
              ...
              <p className="text-[11px] text-mist/60 mt-1.5">
                Only PDF files up to {MAX_CV_SIZE_MB} MB. This copy is shared with the Skills Lab facilitators.
              </p>
              {renderFieldError('cv')}
            </div>
            ...
            <div>
              <Label>Would you like to participate in a mock interview at the Skills Lab?</Label>
              <div className="flex flex-wrap gap-2">
                {['Yes', 'No'].map(o => (
                  <button
                    type="button"
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold border transition ${form.mockInterview === o
                      ? 'bg-orange/20 border-orange text-orange'
                      : 'bg-navy/60 border-mist/15 text-mist/70 hover:border-mist/30'
                    }`}
                  >
        ...
        <Section num="8" title="Additional Questions">
          <div className="space-y-5">
            <div>
              ...
              <div className="space-y-2">
                {HOW_HEARD_OPTIONS.map(o => (
                  ...
                  >
                    <span className="flex-1 text-xs sm:text-sm font-semibold">{o}</span>
                    {form.heardAboutCareerFair === o && <CheckCircle2 className="w-4 h-4 text-orange" />}
                  </button>
                ))}
              </div>
              {form.heardAboutCareerFair === 'Other' && (
                <div className="mt-2.5 bg-navy/70 border border-mist/15 rounded-xl p-3.5 space-y-1.5 animate-fadeIn">
                  ...
                  <input
                    ...
                    onChange={e => update('heardAboutCareerFairOther', e.target.value)}
                    className={baseInputClass}
                  />
              ...
              <div className="flex flex-wrap gap-2">
                {['Yes', 'No'].map(o => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold border transition ${form.attendedLastYear === o
                      ? 'bg-orange/20 border-orange text-orange'
                      : 'bg-navy/60 border-mist/15 text-mist/70 hover:border-mist/30'
                    }`}
                  >
            ...
            <div>
              <Label>Do you have any questions you would like the facilitators to address?</Label>
              <textarea
                rows={3}
                placeholder="Optional — write your questions here"
                value={form.facilitatorQuestions}
                onChange={e => update('facilitatorQuestions', e.target.value)}
