...
export const TicketView: React.FC<TicketViewProps> = ({ participant, onBack, config }) => {
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-6 animate-fadeIn">
      ...
      <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight mb-1">Your Event Ticket</h1>
      <p className="text-xs text-mist/60 mb-5">
        Present this at the entrance on event day. Show your code or let the desk scan the QR code.
      </p>

      {/* Ticket card */}
      <div className="bg-navy border-2 border-orange/40 rounded-3xl overflow-hidden shadow-2xl mb-5">
        <div className="p-5 space-y-4">
          <div className="flex items-center justify-between gap-2">
            <div className="inline-flex items-center gap-1.5 text-xs font-bold text-orange">
              <Ticket className="w-4 h-4" />
              <span>ADMIT ONE</span>
            </div>
            <span className="text-[11px] text-mist/60">{cfg.eventName}</span>
          </div>

          <div className="bg-white p-4 rounded-2xl inline-flex self-center mx-auto">
            <QRCodeSVG value={`CAREERFAIR:${participant.code}:${participant.fullName}`} size={160} level="M" />
          </div>

          <div className="text-center">
            <p className="text-2xl font-mono font-black text-white tracking-wider">{participant.code}</p>
            <p className="text-sm font-bold text-mist mt-2">{participant.fullName}</p>
            <p className="text-[11px] text-mist/60">{participant.email}</p>
          </div>
        </div>

        <div className="border-t border-dashed border-orange/40 p-4 grid grid-cols-2 gap-3 text-xs text-mist/80">
        ...
        <div className="bg-navy/80 p-4 border-t border-mist/15 flex items-center gap-2 text-[11px] text-mist/60">
